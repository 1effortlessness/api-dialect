const
  ...
  {errors, envIsProduction} = require(`${mainPath}/config/default`),
  common = require('../common/basic')

let {Sequelize} = require(`${mainPath}/models`)
...
class ApiDialect {
  ...
  setArgs (args) {
    ...
    let typeSet = {
      ...
      array: i => i.includes(',') ? i.trim().split(',') : _.toArray(i),
      json: JSON.parse
    }
  ...
  error (err) {
    ...
    // 如果是 '1,msg' 格式的
    if (_.isString(err.message) && err.message.split(',').length !== 1) {
      code = err.message.split(',')[0]
      msg = `${errors[code]} --- ${err.message.split(',')[1]}`
    } else {
      code = err.message
      msg = errors[code] === undefined ? err.message : errors[code]
    }
  ...
  if (_.isArray(data)) {
    data = data.map(i => {
      if (i instanceof Sequelize.Instance) {
        return common.clearRedundancyFields(i)
    ...
    Object.keys(data).forEach(k => {
      if (data[k] instanceof Sequelize.Instance) {
        data[k] = common.clearRedundancyFields(data[k])
      } else if (_.isObject(data[k]) && !_.isDate(data[k])) {
        data[k] = _dataStringify(data[k])
      }
    })
  } else if (data instanceof Sequelize.Instance) {
    data = common.clearRedundancyFields(data)
  }
