...
function _dataStringify (obj) {
  ...
  if (_.isArray(data)) {
    data = data.map(i => {
      if (i.toString().includes('SequelizeModel')) {
        return i.toString()
  ...
  } else if (_.isObject(data) && !data.toString().includes('SequelizeModel')) {
    Object.keys(data).forEach(k => {
      if (_.isEmpty(data[k])) {
        return
      }
      if (data[k].toString().includes('SequelizeModel')) {
        data[k] = data[k].toJSON()
      } else if (_.isObject(data[k]) && !_.isDate(data[k])) {
        data[k] = _dataStringify(data[k])
      }
    })
  } else if (data.toString().includes('SequelizeModel')) {
    data = data.toJSON()
  }
